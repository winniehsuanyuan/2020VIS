...
(function (d3) {
  ...
  const margin = { top: 10, right: 10, bottom: 30, left: 50 };//left150
  const innerWidth = width - margin.left - margin.right;
  const innerHeight = height - margin.top - margin.bottom; 
  const g = svg.append('g')
       .attr('transform', `translate(${margin.left},${margin.top})`);
  const color = ['rgba(0, 0, 0, 0.65)', 'rgba(153, 76, 0, 1)', 'rgba(255,153,51,1)'];
  //const markets = ['三重區', '台中市', '台北一', '台北二', '台東市', '嘉義市', '宜蘭市', '東勢鎮', '板橋區', '桃農', '豐原區', '高雄市', '鳳山區', '西螺鎮', '屏東市', '溪湖鎮', '南投市', '花蓮市'];
  var start_time = new Date(2012, 2, 1);
  var end_time = new Date(2019, 10, 1);
